          ...
          <div className="modal-card" style={{maxWidth:560}} onClick={e => e.stopPropagation()}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
              ...
              <div>
                ...
                <input ref={photoRef} id="prod-photo" name="photo" type="file" accept="image/*" style={{display:"none"}} onChange={handlePhotoFile}/>
            ...
            <div className="modal-grid">
              <div className="modal-field" style={{gridColumn:"1/-1"}}><label htmlFor="prod-name">Product Name *</label><input id="prod-name" name="name" type="text" autoComplete="off" placeholder={form.top_type==="shoes" ? "Nike Air Force 1 White" : "Slim Fit Black Shirt"} value={form.name} onChange={e => setForm({...form, name:e.target.value})}/></div>
              <div className="modal-field"><label htmlFor="prod-brand">Brand / Type</label><input id="prod-brand" name="brand" type="text" autoComplete="off" value={form.brand} readOnly style={{opacity:.7}}/></div>
              <div className="modal-field"><label htmlFor="prod-category">Category / Model</label><input id="prod-category" name="category" type="text" autoComplete="off" value={form.category} onChange={e => setForm({...form, category:e.target.value})} placeholder="e.g. Air Force 1, Slim Fit"/></div>
              <div className="modal-field"><label htmlFor="prod-size">Size *</label><select id="prod-size" name="size" autoComplete="off" value={form.size} onChange={e => setForm({...form, size:e.target.value})}>{sizeOpts.map(s => <option key={s}>{s}</option>)}</select></div>
              <div className="modal-field"><label htmlFor="prod-color">Color / Variant</label><input id="prod-color" name="color" type="text" autoComplete="off" placeholder="White, Black, Blue…" value={form.color} onChange={e => setForm({...form, color:e.target.value})}/></div>
              <div className="modal-field"><label htmlFor="prod-stock">Stock Quantity *</label><input id="prod-stock" name="stock" type="number" min="0" autoComplete="off" placeholder="0" value={form.stock} onChange={e => setForm({...form, stock:e.target.value})}/></div>
              <div className="modal-field"><label htmlFor="prod-price">Min Price (KES) *</label><input id="prod-price" name="min_price" type="number" min="0" autoComplete="off" placeholder="4800" value={form.min_price} onChange={e => setForm({...form, min_price:e.target.value})}/></div>
              <div className="modal-field" style={{gridColumn:"1/-1"}}><label htmlFor="prod-sku">SKU *</label><div style={{display:"flex",gap:8,alignItems:"center"}}><input id="prod-sku" name="sku" type="text" autoComplete="off" placeholder="NK-AF1-W42" value={form.sku} onChange={e => setForm({...form, sku:e.target.value})}/><button className="modal-cancel" style={{fontSize:12,padding:"6px 10px",height:"auto"}} onClick={() => { const sku = generateSKU({ brand: form.brand, subType: form.category, color: form.color || "Default", size: form.size }); setForm({...form, sku}); }}>🤖 Auto SKU</button></div></div>
            </div>
