...
const getSizeOpts = (topType, categoryName = "") => {
  ...
  // 🚨 SAFE FALLBACK - Return empty array instead of wrong sizes
  return [];
};
...
export default function Inventory() {
  ...
  const addColor = () => { 
    ...
    if (c && !bulkForm.colors.includes(c)) 
      setBulkForm(f => ({ ...f, colors: [...f.colors, c] })); 
  };
  ...
  // ✅ FIXED: Strict size options for bulk modal
  ...
  const generateBulkPreview = () => {
    if (!bulkForm.name || !bulkForm.brand || !bulkForm.minPrice) { 
      setBulkError("Name, brand, and min price required"); 
      return; 
    }
  ...
  // ── Product modal ─────────────────────────────────────────────
  const openAdd = () => {
    // 🔥 CRITICAL: Ensure topType is always defined
    const topType = (cat.topType || "").toLowerCase();
    
    // 🚨 Guard clause - prevent opening with invalid type
    if (!topType || (topType !== "shoes" && topType !== "clothes")) {
      setFormError("Please select a category first (Shoes or Clothes)");
      return;
    }
    ...
    // ✅ Get correct sizes based on type AND category
    const sizes = getSizeOpts(topType, f.category);
    
    // 🛡️ Safety check - prevent empty size options
    if (sizes.length === 0) {
      setFormError(`No size options available for ${topType}${f.category ? ` / ${f.category}` : ''}`);
      return;
    }
    
    // ✅ Only set size if current size isn't valid for this type
  ...
  const save = async () => {
    if (!form.name || !form.sku || !form.min_price) { 
      setFormError("Name, SKU and Min Price are required"); 
      return; 
    }
    setSaving(true); 
    setFormError("");
  ...
  const openCatEdit = (topType, label) => {
    ...
    setCatEditId(topType); 
    setCatError(""); 
    setCatModal(true);
  };
  
  const saveCat = async () => {
    if (!catForm.name || !catForm.top_type) { 
      setCatError("Name and type are required"); 
      return; 
    }
    setCatSaving(true); 
    setCatError("");
    try {
      ...
      setCatError(e.response?.data?.error || "Save failed"); 
    } finally { 
      setCatSaving(false); 
    }
  };
  ...
  const saveBrand = async () => {
    if (!brandForm.name || !brandForm.top_type) { 
      setBrandError("Name and type are required"); 
      return; 
    }
    setBrandSaving(true); 
    setBrandError("");
  ...
  const saveSubtype = async () => {
    if (!subtypeForm.name || !subtypeForm.brand_id) { 
      setSubtypeError("Name and brand are required"); 
      return; 
    }
            ...
            <div className="modal-grid">
              ...
              <div className="modal-field" style={{gridColumn:"1/-1"}}>
                ...
                <div style={{display:"flex",gap:6,marginTop:6,alignItems:"center"}}>
                  <select onChange={e => { if(e.target.value) { addSizeFromPicker(e.target.value); e.target.value=""; }}} style={{flex:1,padding:"6px 8px",borderRadius:6,border:"1px solid var(--border)",background:"var(--bg2)",color:"var(--text1)",fontSize:13}}>
                    <option value="">+ Pick size to add…</option>
                    {bulkUnselectedSizes.map(s => <option key={s} value={s}>{cat.topType === "shoes" ? `EU ${s}` : (s.match(/^\d+$/) ? `Waist ${s}"` : s)}</option>)}
                  </select>
                  ...
                  <button className="modal-cancel" style={{fontSize:11,padding:"6px 10px",whiteSpace:"nowrap"}} onClick={() => setBulkForm(f => ({...f, sizes: []}))}>Clear</button>
                </div>
              </div>
              <div className="modal-field">
                <label>Min Price (KES) *</label>
                          ...
                          {isAdmin && (
                            <td>
                              <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                                ...
                                <button className="tbl-btn" style={{background:"var(--bg2)",color:"var(--text2)",border:"1px solid var(--border)"}} onClick={() => { 
                                ...
                                <button className="tbl-btn tbl-btn--edit" onClick={() => { 
                                  setBarcodeSku(p.sku); 
                                  setBarcodeName(`${p.name} ${p.color || ''} Sz${p.size}`); 
                                }}>📄 Barcode</button>
                                <button className="tbl-btn tbl-btn--del" onClick={() => { 
                                  setDelId(p.id); 
                                  setDelName(`${p.name} Sz${p.size}`); 
                                }}>🗑</button>
                              </div>
                            </td>
                          )}
